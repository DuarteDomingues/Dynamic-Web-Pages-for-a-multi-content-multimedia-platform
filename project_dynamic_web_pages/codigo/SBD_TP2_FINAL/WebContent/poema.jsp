...
<section class="mainBody">
	...
	if (session.getAttribute("idUser") != null && session.getAttribute("idUser") != "") {
		...
		String query = "SELECT * from sbd1.recurso_multimedia inner join sbd1.poema  on poema.idRecurso = recurso_multimedia.idRecurso inner join sbd1.user on user.idUser = recurso_multimedia.userCriador where sbd1.recurso_multimedia.idRecurso = "
		+ recId + "";
		...
		while (rs.next()) {
			...
			poemaType = rs.getString("recursoTipo");
		...
		//######################################################################################
		String queryp = "select conteudo from sbd1.recurso_multimedia where idRecurso = " + recId;

		ResultSet rsPoema = db.makeStatement(queryp);
		...
		while (rsPoema.next()) {
			...
			poema = rsPoema.getString("conteudo");
	...
	<%
		if (session.getAttribute("LogedInUserType").equals("3")) {
			...
			<form action="${pageContext.request.contextPath}/Manage_blockContent"
					...
					<input type="hidden" name="recursoTipo" value=<%=poemaType%> />
	...
	ArrayList<String> poetas = dl.getPoetasNames(idRecursoS);
	ArrayList<Integer> poetasIds = dl.getPoetasIds(idRecursoS);
	%>
	<h2>Poets:</h2>
	<%
	for (int i = 0; i < poetas.size(); i++) {
		int idPoeta = poetasIds.get(i);
	%>
	<h3><%=poetas.get(i)%></h3>
	...
	<form action="${pageContext.request.contextPath}/Manage_Artista"
		...
		<input name="consultar" type="submit" value="Consult" /> <input
			type="hidden" name="idArtista" value="<%=idPoeta%>" /> <input
...
///////COMMENTS
		...
		for ( int x =0; x<comments.size();x++){
			...
			String comentario = comments.get(x);
			Integer comentarioId = commentsIds.get(x);
			String userComentador = dl.getNameUser(comentarioId);
			String dateCom = commentDates.get(x);
	...
	<form action="${pageContext.request.contextPath}/Manage_comment"
		...
		<input type="hidden" name="userComentador" value=<%=idUser%> /> <input
			...
			type="hidden" name="recursoTipo" value="<%=poemaType%>" /> <input
			type="hidden" name="recursoTitulo" value="<%=poemaType%>" /> <br>
	...
	if (dicClass!=null){
		 Iterator hmIterator = dicClass.entrySet().iterator();
			...
			while (hmIterator.hasNext()) {
				...
				int ix = (int) mapElement.getKey();
				int rating = (int) mapElement.getValue();
				String userComentador = dl.getNameUser(ix);
				String userS = String.valueOf(ix);
	...
	<form action="${pageContext.request.contextPath}/Manage_comment"
		...
		<div>
			<input id="class" type="number" name="classification" min="0"
				...
				name="recursoTitulo" value="<%=poemaType%>" /> <input
				type="hidden" name="recursoTipo" value="<%=poemaType%>" />

		</div>
		<div>
			<input type="submit" value="rate this poem">
		</div>
	</form>
	<%
	...
	%>
		...
		HashMap<Integer, String> dic = dl.getRecursoNameDic(titulo, recId);
		...
		<form action="${pageContext.request.contextPath}/Manage_association"
			...
			<div id="list1" class="dropdown-check-list" tabindex="100">
				...
				<ul class="items">
					...
					<input type="hidden" name="idRecurso" value=<%=recId%> />
					<input type="hidden" name="tipoRecurso" value=<%=poemaType%> />
					<%
					while (hmIterator.hasNext()) {
						Map.Entry mapElement = (Map.Entry) hmIterator.next();
						int ind = (int) mapElement.getKey();

						String titulop = (String) mapElement.getValue();
					%>

					<li><input type="checkbox" name=<%=ind%> value=<%=ind%> /> <%=titulop%></li>
					<%
					}
					%>
				</ul>
	...
	<br>
		...
		while (hmAssociacoesIterator.hasNext()) {
			...
			String titulox = (String) mapElement.getValue();
		%>
