...
<body>
	...
	if (session.getAttribute("idUser") != null && session.getAttribute("idUser") != "") {
		...
		String query = "SELECT * from sbd1.recurso_multimedia inner join sbd1.musica  on musica.idRecurso = recurso_multimedia.idRecurso inner join sbd1.artista on artista.idArtista = musica.idAutor inner join sbd1.user on user.idUser = recurso_multimedia.userCriador where sbd1.recurso_multimedia.idRecurso = "
		+ recId + "";

		DatabaseConnector db = new DatabaseConnector();
		ResultSet rs = db.makeStatement(query);
		String tituloMusica = null;
		...
		while (rs.next()) {
			tituloMusica = rs.getString("titulo");
			...
			musicaType = rs.getString("recursoTipo");
			idAutorLetra = rs.getInt("idAutorLetra");
	...
	<%
		if (session.getAttribute("LogedInUserType").equals("3")) {
			...
			<form action="${pageContext.request.contextPath}/Manage_blockContent"
					...
					<input type="hidden" name="recursoTipo" value=<%=musicaType%> />
	...
	<form action="${pageContext.request.contextPath}/Manage_Artista"
		...
		<input name="consultar" type="submit" value="Consult" /> <input
			type="hidden" name="idArtista" value=<%=artistaId%> /> <input
			type="hidden" name="artistaTag" value="RLZ" />

	</form>
	
	<%
	String nameAutorLetra = db.getNomeAutorLetra(idAutorLetra);
	...
	<p><%=nameAutorLetra%></p>
		...
		method="get">
		<input name="consultar" type="submit" value="Consult" /> <input
			type="hidden" name="idArtista" value=<%=idAutorLetra%> /> <input
			type="hidden" name="artistaTag" value="RLZ" />
...
///////COMMENTS
	...
	<form action="${pageContext.request.contextPath}/Manage_comment"
		...
		<input type="hidden" name="userComentador" value=<%=idUser%> /> <input
			...
			type="hidden" name="recursoTipo" value="<%=musicaType%>" /> <input
			type="hidden" name="recursoTitulo" value="<%=tituloMusica%>" /> <br>
...
<!--      CLASSIFICACOES -->
	...
	<form action="${pageContext.request.contextPath}/Manage_comment"
		...
		<div>
			<input id="class" type="number" name="classification" min="0"
				...
				name="recursoTitulo" value="<%=musicaType%>" /> <input
				type="hidden" name="recursoTipo" value="<%=musicaType%>" />

		</div>
		<br>
		<div>
			<input type="submit" value="rate this music">
		</div>
	</form>
	<br>
	<%
	...
	%>
		...
		HashMap<Integer, String> dic = dl.getRecursoNameDic(tituloMusica, recId);
		...
		<form action="${pageContext.request.contextPath}/Manage_association"
			...
			<div id="list1" class="dropdown-check-list" tabindex="100">
				...
				<ul class="items">
					<input type="hidden" name="titleRecurso" value=<%=tituloMusica%> />
					<input type="hidden" name="idRecurso" value=<%=recId%> />
					<input type="hidden" name="tipoRecurso" value=<%=musicaType%> />
					<%
					while (hmIterator.hasNext()) {
						Map.Entry mapElement = (Map.Entry) hmIterator.next();
